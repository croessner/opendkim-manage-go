...
func PlanRotation(plan RotationPlan) (*Generation, error) {
	if plan.Current == nil || plan.Current.Number() == ^uint64(0) ||
		...
		plan.Random == nil || ValidateIdentifier(plan.TenantID) != nil ||
	...
	for index := range requested {
		selectors[index], err = allocateHistoricalID("dkim2", plan.Random, usedSelectors, plan.History.SelectorUsed)
		if err != nil {
			return nil, err
		}
		handles[index], err = allocateHistoricalID("key", plan.Random, usedHandles, plan.History.HandleUsed)
		if err != nil {
			return nil, err
		}
	}
...
func allocateHistoricalID(prefix string, random io.Reader, used map[string]struct{}, historical func(string) (bool, error)) (string, error) {
	for range rotationAllocationAttempts {
		buffer := make([]byte, 12)
...
// SelectOneEligibleBinding makes a pure deterministic batch-one decision from complete retained lineage.
func SelectOneEligibleBinding(now time.Time, current *Generation, history LineageHistory, rotateAfterDays, maxClockSkewSeconds int) (EligibilityDecision, error) {
	if current == nil || now.Location() != time.UTC || !history.Complete || rotateAfterDays < 1 || maxClockSkewSeconds < 0 {
		return EligibilityDecision{}, ErrInvalid
	}
	...
	for _, policy := range current.policies {
		...
		for _, credential := range credentials {
			...
			seenGeneration := make(map[uint64]struct{})
			for _, fact := range history.Facts {
				...
				if _, duplicate := seenGeneration[fact.generation]; duplicate {
					return EligibilityDecision{}, ErrInvalid
				}
				seenGeneration[fact.generation] = struct{}{}
				created := parseGeneralizedTime(fact.created)
				if created.IsZero() || created.After(now.Add(time.Duration(maxClockSkewSeconds)*time.Second)) {
					return EligibilityDecision{}, ErrInvalid
				}
		...
		if !bindingSince.Add(time.Duration(rotateAfterDays) * 24 * time.Hour).After(now) {
	...
	slices.SortFunc(due, func(left, right EligibilityDecision) int {
		leftKey, rightKey := left.tenant+"\x00"+left.domain+"\x00"+string(left.use), right.tenant+"\x00"+right.domain+"\x00"+string(right.use)
		switch {
